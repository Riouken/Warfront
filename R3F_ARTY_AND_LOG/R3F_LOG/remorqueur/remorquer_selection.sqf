...
else
{
	...
	if (!(isNull _objet) && (alive _objet) && !(_objet getVariable "R3F_LOG_disabled")) then
	{
		...
		{
			if ( (_objet distance _remorqueur <= 30) && (isNull (_remorqueur getVariable "R3F_LOG_est_transporte_par")) )then
			{
				...
				// Attacher à l'arrière du véhicule au ras du sol (on the ground at the back of the vehicle)
