...
//EGG A3
			EGG_EVO_pweaponitems = primaryWeaponItems player;
			player removeWeapon _arme_principale;
		}
		else {sleep 0.5;};
...
//EGG A3
				{player addprimaryweaponitem _x} forEach EGG_EVO_pweaponitems;
				player addMagazine (getArray(configFile>>"CfgWeapons">>_arme_principale>>"magazines") select 0); // add primary weapon mag
				_mz = getArray(configFile>>"CfgWeapons">>_arme_principale>>"muzzles");
				{ if (_x != "this") then { player addMagazine (getArray(configFile>>"CfgWeapons">>_arme_principale>>_x>>"magazines") select 0); }; } forEach _mz; // add one mag for each muzzle   
