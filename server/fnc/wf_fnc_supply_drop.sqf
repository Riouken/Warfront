...
{    
  ...
  if (!(_side in ["East","West"])) then {_side = "West";};
  	case 0: {_paraName = "NonSteerable_Parachute_F";              //format["Parachute%1",_side],;}; //ParachuteMediumEast or ParachuteMediumWest not "FLY" (that spawns 150 m above ground) }; 
  	case 1: {_paraName = "NonSteerable_Parachute_F";              //format["ParachuteMedium%1",_side];}; //ParachuteMediumEast or ParachuteMediumWest not "FLY" (that spawns 150 m above ground) }; 
  	case 2: {_paraName = "NonSteerable_Parachute_F";              //format["ParachuteBig%1",_side];};
...
["wf_addAction",[_crate,"WF- FOB Supplies","client\gui\wf_gui_supply_start.sqf"]] call CBA_fnc_globalEvent;

if (count wf_crates_action_list == 2) then {wf_crates_action_list = [];};

wf_crates_action_list = wf_crates_action_list + [_crate];

publicVariable "wf_crates_action_list";

_smoke = [_crate] spawn wf_fnc_supply_smoke;
